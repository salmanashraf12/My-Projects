class Rotor:
    ...
    def input_wiring(self):
        wiring = input("Enter rotor wiring (26 uppercase letters): ").upper()
        self.right = wiring

    def input_notch(self):
        notch = input("Enter notch setting for the rotor: ").upper()
        if len(notch) == 1 and notch.isalpha():
            self.notch = notch
        else:
            print("Invalid input. Notch must be a single alphabetical character.")
